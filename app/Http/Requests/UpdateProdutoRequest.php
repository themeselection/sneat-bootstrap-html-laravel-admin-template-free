...
class UpdateProdutoRequest extends FormRequest
{
    ...
    {
        $produtoId = (int) $this->route('produto')->id;

        return [
            'sku' => ['required', 'string', 'max:40', Rule::unique('produtos', 'sku')->ignore($produtoId)],
            'nome' => ['required', 'string', 'max:255'],
            'descricao' => ['nullable', 'string'],
            'codigo_barras' => ['nullable', 'string', 'max:50', Rule::unique('produtos', 'codigo_barras')->ignore($produtoId)],
