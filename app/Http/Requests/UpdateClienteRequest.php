...
class UpdateClienteRequest extends FormRequest
{
    ...
    {
        $clienteId = (int) $this->route('cliente')->id;

        return [
            'codigo' => ['nullable', 'string', 'max:20', Rule::unique('clientes', 'codigo')->ignore($clienteId)],
            'tipo_pessoa' => ['required', Rule::in(['PF', 'PJ'])],
            'nome' => ['required', 'string', 'max:255'],
            'nome_fantasia' => ['nullable', 'string', 'max:255'],
            'cpf_cnpj' => ['required', 'string', 'min:11', 'max:14', Rule::unique('clientes', 'cpf_cnpj')->ignore($clienteId)],
