...
<div class="d-flex justify-content-between align-items-center mb-6">
    <div>
        <h4 class="mb-1">Financeiro - Contas a Pagar</h4>
        <p class="mb-0 text-muted">Controle de pagamentos de fornecedores e estornos.</p>
    </div>
    <a href="{{ route('contas-pagar.export-csv', request()->query()) }}" class="btn btn-outline-primary">Exportar CSV</a>
</div>
...
<div class="row g-4 mb-6">
    ...
    <div class="col-md-4"><div class="card border-start border-success border-3"><div class="card-body"><small class="text-muted">Pago no mês</small><h4 class="mb-0">R$ {{ number_format($cards['pago_mes'], 2, ',', '.') }}</h4></div></div></div>
</div>

<div class="card mb-6">
    <div class="card-body">
        <form method="GET" action="{{ route('contas-pagar.index') }}" class="row g-3">
            <div class="col-md-2">
        ...
        <table class="table table-hover">
            ...
            <tbody>
                @forelse ($contas as $conta)
                    <tr>
                        <td>{{ $conta->compra->numero ?? ('#'.$conta->id) }}</td>
                        ...
                        <td>
                            <a href="{{ route('contas-pagar.show', $conta) }}" class="btn btn-sm btn-outline-primary mb-2">Ver</a>
                            @perm('financeiro.contas_pagar.pagar')
                                <div class="d-flex flex-wrap gap-2 mb-2">
                                    <form method="POST" action="{{ route('contas-pagar.pagar', $conta) }}" class="d-flex gap-1">
                                        @csrf
                                        ...
                                        <select name="forma_pagamento" class="form-select form-select-sm" style="width: 120px">
                                            ...
                                            @foreach (['PIX','BOLETO','TED','DINHEIRO','CARTAO'] as $forma)
                                                <option value="{{ $forma }}">{{ $forma }}</option>
                                            @endforeach
                                        </select>
                                        <button class="btn btn-sm btn-outline-success" @disabled($conta->status === 'QUITADO' || $conta->status === 'CANCELADO')>Pagar</button>
                                    </form>
                                </div>
                            @endperm
                            @perm('financeiro.contas_pagar.estornar')
                                <div>
                                    <form method="POST" action="{{ route('contas-pagar.estornar', $conta) }}" class="d-flex gap-1">
                                        @csrf
                                        <input type="hidden" name="data_movimento" value="{{ now()->format('Y-m-d H:i:s') }}">
                                        <input type="number" name="valor" step="0.01" min="0.01" class="form-control form-control-sm" style="width: 90px" placeholder="Valor">
                ...
                @empty
                    <tr>
                        <td colspan="7" class="text-center text-muted py-5">Nenhuma conta a pagar encontrada.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
