...
@section('title', 'Detalhe da Conta a Pagar')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-6">
    <div>
        <h4 class="mb-1">Conta #{{ $conta->id }}</h4>
        <p class="mb-0 text-muted">Compra: {{ $conta->compra->numero ?? '-' }} • Status: {{ $conta->status }}</p>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('contas-pagar.extrato.export-csv', $conta) }}" class="btn btn-outline-primary">Exportar Extrato CSV</a>
        <a href="{{ route('contas-pagar.index') }}" class="btn btn-outline-secondary">Voltar</a>
    </div>
</div>
...
<div class="row g-6">
    <div class="col-lg-4">
        <div class="card mb-6">
            ...
            <div class="card-body">
                <p class="mb-2"><strong>Fornecedor:</strong><br>{{ $conta->fornecedor->nome ?? '-' }}</p>
                    ...
                    <tbody>
                        @forelse ($conta->movimentos->sortByDesc('data_movimento') as $mov)
                            <tr>
                                ...
                                <td><span class="badge bg-label-{{ $mov->tipo === 'PAGAMENTO' ? 'success' : 'danger' }}">{{ $mov->tipo }}</span></td>
